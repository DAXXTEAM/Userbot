    ...
    try:
        ...
        # ─── .mail chatpdf ───
            ...
            async with aiohttp.ClientSession() as session:
                ...
                await message.edit_text("🤖 Calling ChatPDF gen API...")

                # Step 2: Call ChatPDF gen endpoint
                try:
                    async with session.get(
                        "http://155.103.70.111:5000/api/chatgpt/gen",
                        timeout=aiohttp.ClientTimeout(total=120)
                    ) as resp:
                        if resp.status == 200:
                            result = await resp.json()
                            checkout_url = result.get("url", result.get("checkout_url", "N/A"))
                            acc_email = result.get("email", email)
                            acc_pass = result.get("password", password)
                            ...
                            await message.edit_text(
                                f"❌ ChatPDF error (HTTP {resp.status}):\n`{err_text[:200]}`\n\n"
                                f"📧 Mail active: `{email}`"
                            )
                            mail_sessions[chat_id] = {"email": email, "password": password, "token": mail_token, "account_id": account_id}
                except asyncio.TimeoutError:
                    await message.edit_text(
                        f"⏱️ Timeout — try `.mail chatpdf` again\n\n"
                        f"📧 Mail active: `{email}`\nPassword: `{password}`"
                    )
                    mail_sessions[chat_id] = {"email": email, "password": password, "token": mail_token, "account_id": account_id}
                except Exception as api_err:
                    await message.edit_text(
                        f"❌ ChatPDF API error: `{api_err}`\n\n"
                        f"📧 Temp email active: `{email}`\n"
                        f"Password: `{password}`"
                    )
                    mail_sessions[chat_id] = {
                        "email": email,
            ...
            await message.edit_text(
                "⚠️ Unknown mail command. Use `.mail` for help."
            )

    except Exception as e:
        await message.edit_text(f"❌ Mail error: `{e}`")
...
async def promote_cmd(client, message: Message):
    try:
        ...
        await message.edit_text(f"❌ Promote error: `{e}`")
...
async def demote_cmd(client, message: Message):
    try:
        ...
        await message.edit_text(f"❌ Demote error: `{e}`")
...
async def title_cmd(client, message: Message):
    try:
        ...
        await message.edit_text(f"✅ **{user_name}** ka title set: `{title}`")
    except Exception as e:
        await message.edit_text(f"❌ Title error: `{e}`")
...
async def tmute_cmd(client, message: Message):
    try:
        ...
        reply = message.reply_to_message
        if not reply:
            await message.edit_text("⚠️ Reply to a user! `.tmute 10m`")
            return
        user_id = reply.from_user.id
        user_name = reply.from_user.first_name
        ...
        if len(args) > 1:
            ...
            elif t.isdigit(): duration = int(t) * 60
        ...
        await message.edit_text(f"❌ TMute error: `{e}`")
...
async def warn_cmd(client, message: Message):
    try:
        ...
        await message.edit_text(f"❌ Warn error: `{e}`")
...
async def unwarn_cmd(client, message: Message):
    try:
        reply = message.reply_to_message
        if not reply:
            await message.edit_text("⚠️ Reply to a user to unwarn!")
            return
        user_id = reply.from_user.id
        user_name = reply.from_user.first_name
        key = f"{message.chat.id}_{user_id}"
        if hasattr(warn_cmd, "warns") and key in warn_cmd.warns:
            warn_cmd.warns[key] = max(0, warn_cmd.warns[key] - 1)
        await message.edit_text(f"✅ **{user_name}** ka 1 warn remove ho gaya!")
    except Exception as e:
        await message.edit_text(f"❌ Unwarn error: `{e}`")

@app.on_message(filters.me & filters.command("kick", prefixes="."))
async def kick_group_cmd(client, message: Message):
    try:
        reply = message.reply_to_message
        if not reply:
            await message.edit_text("⚠️ Reply to a user to kick!")
            return
        user_id = reply.from_user.id
        user_name = reply.from_user.first_name
        await client.ban_chat_member(message.chat.id, user_id)
        await client.unban_chat_member(message.chat.id, user_id)
        await message.edit_text(f"👢 **{user_name}** kicked!")
    except Exception as e:
        await message.edit_text(f"❌ Kick error: `{e}`")
...
async def unban_cmd(client, message: Message):
    try:
        reply = message.reply_to_message
        if not reply:
            await message.edit_text("⚠️ Reply to a user to unban!")
            return
        user_id = reply.from_user.id
        user_name = reply.from_user.first_name
        await client.unban_chat_member(message.chat.id, user_id)
        ...
        await message.edit_text(f"❌ Unban error: `{e}`")
...
async def vpn_gen_cmd(client, message: Message):
    try:
        ...
        # Step 3: Get CSRF from plan page
        r2 = s.get("https://1vpn.org/select_plan/", headers=_headers, timeout=15)
        csrf2 = _BS(r2.text, "html.parser").find("input", {"name": "csrfmiddlewaretoken"})["value"]

        # Step 4: Submit plan + stripe
        r3 = s.post("https://1vpn.org/payments/select_method/",
            headers={**_headers, "Referer": "https://1vpn.org/select_plan/", "Origin": "https://1vpn.org"},
        ...
        if checkout_url:
            ...
            await message.edit_text(
                f"⚠️ Account created but checkout URL not found!\n\n"
                f"📧 **Email:** `{email}`\n"
                f"🔑 **Password:** `{password}`\n"
                f"🌐 Login: https://1vpn.org/select_plan/"
            )

    except Exception as e:
        await message.edit_text(f"❌ 1VPN error: `{e}`")



# ==========================================
# 🔧 FREECAD CHECKOUT GENERATOR
# ==========================================
...
async def freecad_cmd(client, message: Message):
    try:
        ...
        r = _req.get(
            "https://www.freecad.org/stripe-checkout-session.php",
        ...
        if "checkout.stripe.com" in checkout_url or "pay/" in checkout_url:
            await message.edit_text(
                f"✅ **FreeCAD Checkout Generated!**\n\n"
                f"💰 **Amount:** `${amount} USD`\n"
                f"🌐 **Site:** `freecad.org`\n\n"
                f"💳 **Checkout URL:**\n`{checkout_url}`",
            ...
            await message.edit_text(f"❌ Could not generate checkout.\nStatus: {r.status_code}")

    except Exception as e:
        await message.edit_text(f"❌ FreeCAD error: `{e}`")
